...
//			}
//
//			Optional<UserAccounts> userAccounts = userAccountsRepository.findById(useridValue);
//
...
//	}
	...
	public Boolean bookingRoom(String userid, Integer roomid, String reservetime, String endtime, String eventid) {
		...
		try {
			...
			System.out.println("accId: " + userAccountId);
			System.out.println("Uid " + useridValue);
			Integer eventidValue = null;
			if (!eventid.isEmpty()) {
				eventidValue = Integer.parseInt(eventid);
			} else {
...
//						System.out.println("E: " + data.getEvent().getId());
//						//Date[] formated = convertDate.parseStringtoDate(reversetime, endtime, reversetime);
//						System.out.println("ketqua: " + reversetime.compareTo(new java.util.Date()));
//						if (reversetime.compareTo(new java.util.Date()) >= 0
//								&& reversetime.compareTo(endtime) < 0) {
//							data.setReserveTime(reversetime);
...
//	}
	...
	@Override
	public BookingDetailDTO getBookingDetailById(Integer detailId) {
		...
		if (optionalDetail.isPresent()) {
			data = optionalDetail.get();
			detailDTO = detailConvert.toBDDTO(data);
			return detailDTO;
		} else {
			System.out.println("Không tìm thấy chi tiết");
			return null;
	...
	@Override
	public List<DetailDTO> getExpectedRoomHistoryBooking(int roomId) {
		List<DetailDTO> result = new ArrayList<DetailDTO>();
		List<Details> list = detailRepository.listBookingStatuslike2(roomId);
		for(Details d : list) {
			DetailDTO dto = new DetailDTO();
			dto = detailConvert.toDTO(d);
			result.add(dto);
		}
		return result;
	}
