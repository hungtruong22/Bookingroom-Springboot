...
public interface IHistoryOfAdminRepository extends JpaRepository<History, Integer> {
    ...
    //Thống kê theo năm
    @Query("SELECT d.eventId FROM history d WHERE d.eventId = 2 AND d.status = 3 " +
            "AND YEAR(d.reserveTime)=:year GROUP BY d.eventId")
    List<Integer> getEvenByYear(Integer year);

    @Query("SELECT COUNT(*) FROM history d WHERE d.eventId > 1 AND d.status = 3 AND YEAR(d.reserveTime)=:year")
    ...
    ///Thống kê theo tháng
    @Query("SELECT d.eventId FROM history d WHERE d.eventId > 1 AND d.status = 3 " +
