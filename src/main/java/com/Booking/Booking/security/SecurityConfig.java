...
//			authorize.requestMatchers("/admin/history/Statistical/excel/**").permitAll();
			...
			authorize.requestMatchers("/events/list","/roomtypes/list","/notifications/list","/notifications/{id}", "/users/update/{id}", "/detail/expectedRoomBookingHistory/{id}").hasAnyAuthority(AppRole.ADMIN, AppRole.TEACHER);
			authorize.requestMatchers("/users/details").hasAuthority(AppRole.TEACHER);
