...
import com.Booking.Booking.payload.UpdatePayload;
...
//	@PutMapping("/refusebookingroom/{id}")
	...
	@PutMapping("teacher/cancelbookingroom/{id}")
...
//	@PreAuthorize("hasAuthority('teacher')")
//	@PutMapping("/teacher/updatebookingroomwhenwaitingaccept/{id}")
////	@PutMapping("/updatebookingroomwhenwaitingaccept")
//	public ResponseEntity<?> UpdateBookingRoomWhenWaitingAccept(@PathVariable("id") Integer id ,
//																@RequestBody UpdatePayload updatePayload){
//
//
//		System.out.println("pay: " + updatePayload.getId() + " " + updatePayload.getRoomid() + " "
//		+ updatePayload.getReservetime() + " " + updatePayload.getEndtime() + " " + updatePayload.getEventid());
//		ResponseDTO<Boolean> responseDTO = new ResponseDTO<>();
//
//		try {
//			System.out.println("PL: " + updatePayload);
//			Boolean result = detailServiceImpl.updateBookingRoomWhenWatingAccept
//					(updatePayload.getId(), updatePayload.getRoomid(),
//							updatePayload.getEventid(), updatePayload.getReservetime(),
//							updatePayload.getEndtime());
//			responseDTO.setData(result);
//			System.out.println("KQ: " + result);
//			String message = result ? "Chỉnh sửa thành công" : "Chỉnh sửa thất bại";
//			responseDTO.setMessage(message);
...
//	}
@PreAuthorize("hasAuthority('admin') or hasAuthority('teacher')")
@PutMapping("/both/updatebookingroomwhenwaitingaccept")
//	@PutMapping("/updatebookingroomwhenwaitingaccept")
public ResponseEntity<?> UpdateBookingRoomWhenWaitingAccept(@RequestParam("id") String id,
															@RequestParam(name = "roomid" , required = false) String roomid,
															@RequestParam(name = "eventid", required = false) String eventid,
		...
		System.out.println("pay: " + id + " " + roomid + " " + eventid + " "
		+ reservetime + " " + endtime);
	ResponseDTO<Boolean> responseDTO = new ResponseDTO<>();
	try {
		Boolean result = detailServiceImpl.updateBookingRoomWhenWatingAccept
	...
	@PreAuthorize("hasAuthority('admin') or hasAuthority('teacher')")
	@GetMapping("/both/getbookingdetail/{id}")
//	@GetMapping("/getdetail/{id}")
	public ResponseEntity<?> getBookingDetailById(@PathVariable("id") Integer id){
		ResponseDTO<BookingDetailDTO> responseDTO = new ResponseDTO<>();
		try {
			BookingDetailDTO bookingDetailDTO = detailServiceImpl.getBookingDetailById(id);
			responseDTO.setData(bookingDetailDTO);
			String message = bookingDetailDTO != null ? "Thành công" : "Thất bại";
			responseDTO.setMessage(message);
			return bookingDetailDTO != null ? ok(responseDTO) : notFound(bookingDetailDTO);
		} catch (Exception e) {
			System.out.println("Error Get Detail By Id: " + e.getMessage());
			responseDTO.setData(null);
			responseDTO.setMessage("Thất bại");
	...
	@GetMapping("both/expectedRoomBookingHistory/{id}")
	@PreAuthorize("hasAuthority('admin') or hasAuthority('teacher')")
	public ResponseEntity<?> getExpectedRoomBookingHistory(@PathVariable(name = "id", required = true) String id) {
		try {
			int roomId = Integer.parseInt(id);
			
			List<DetailDTO> result = detailServiceImpl.getExpectedRoomHistoryBooking(roomId);
			String message = result.size() > 0 ? "Get expected room booking success": "Get expected room booking failed";
			ResponseDTO<List<DetailDTO>> response = new ResponseDTO<List<DetailDTO>>(message, result);
			
			return result.size() > 0 ? ok(response): notFound(response);
		} catch (Exception e) {
			String message = "Get expected room booking history error";
			ResponseDTO<String> response = new ResponseDTO<String>(message, e.getMessage());
